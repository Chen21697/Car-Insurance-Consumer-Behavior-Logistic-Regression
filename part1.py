...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 21:11:53 2020
...
for i in range(epoch):
    ...
    #loss_train = np.sum(-1*((y_head_train*np.log(sigmoid(y_train))) + ((1-y_head_train)*np.log(1-sigmoid(y_train)))))/sampleNum + lamb*np.sum(w**2)
    #loss_val = np.sum(-1*((y_head_val*np.log(sigmoid(y_val))) + ((1-y_head_val)*np.log(1-sigmoid(y_val)))))/validNum + lamb*np.sum(w**2)
    ...
    # compute ∂L/∂w
    w_grad = ((y_head_train - sigmoid(y_train))@(-x_train))/sampleNum + 2*lamb*w
    # compute ∂L/∂b
